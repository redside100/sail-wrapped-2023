    ...
    with open(f"data/{file_name}", "r", encoding="utf-8") as f:
        ...
        for j, message in enumerate(log_messages):
            ...
            if '.' not in formatted_timestamp:
                formatted_timestamp = formatted_timestamp.replace("+00:00", "") + ".0+00:00"
                
            timestamp = int(datetime.strptime(formatted_timestamp, "%Y-%m-%dT%H:%M:%S.%f%z").timestamp())
            ...
            t = (message_id, type, timestamp, content, author_id, author_name, author_nickname, author_discriminator, author_avatar_url, attachments, embeds, stickers, reactions, total_reactions, mentions, channel_id, channel_name)
            conn.cursor().execute(
